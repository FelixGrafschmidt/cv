<template>
	...
	<div class="flex w-[297mm] gap-8 flex-row py-8 px-12">
		<aside class="w-1/3 flex flex-col gap-6 p-8 h-full bg-gray-200">
			<div class="p-2">
			...
			<div class="flex flex-col">
				<span
					class="mb-4 w-full text-2xl pb-1 text-teal-600 font-semibold border-b-4 border-gray-300 uppercase">{{ $t("skills") }}
				</span>
				<div>
			...
			<div class="flex flex-col">
				...
				<div class="flex flex-row">
					<div class="flex flex-col w-1/2">
						<span v-for="(interest, i) in interests.slice(0, 4)" :key="i" class="p-1">{{ interest }}</span>
					</div>
					<div  class="flex flex-col w-1/2">
						<span v-for="(interest, i) in interests.slice(4, 8)" :key="i" class="p-1">{{ interest }}</span>
					</div>
				</div>
			</div>
		</aside>
				...
				<i18n-t keypath="webVersion" tag="span">
					<template #link>
						<a class="text-blue-500 underline"
							:href="`https://cv.felgraf.dev?language=${$i18n.locale}`">{{ $t("here") }}
							(https://cv.felgraf.dev)</a>
					</template>
				</i18n-t>
